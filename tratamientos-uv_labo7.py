...
def plot_uv(path):
    label = path.replace("tratamiento-uv_", "")
    t, A = np.loadtxt(os.path.join(c.ROOT, f'{path}.txt'), skiprows=1).T
    C = 1 - A/A[0]
    if 'control' in path:
        plt.axhline(np.mean(C))
    else:
        plt.scatter(t, C, label=label)
        plt.plot(t, C)
...
plot_uv('31-10/control-uv_TiO2_fino_vidrio')
...
plt.plot(wavelength, counts / np.max(counts), '.-', label='LED', color='blue')

wavelength, counts = np.loadtxt(os.path.join(c.ROOT, '07-11/espectro-uv/plasma/5s-16V-seco.txt'), skiprows=2).T
plt.plot(wavelength, counts / np.max(counts), '.-', label='Reactor sin agua', color='red')
...
plt.plot(wavelength, counts / np.max(counts), '.-', label='Reactor con agua', color='orange')
#plt.axvline(wavelength[np.argmax(counts)], label=f'$\lambda_{{max}} = {wavelength[np.argmax(counts)]}$', color='blue')
plt.xlabel('$\lambda$')
plt.ylabel('$I$')
plt.legend()
